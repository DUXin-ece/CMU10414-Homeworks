...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    return ndl.ops.summation(ndl.ops.log(ndl.ops.summation(ndl.ops.exp(Z), 1)) - ndl.ops.summation((Z * y_one_hot), 1)) / Z.shape[0]
    ### END YOUR SOLUTION
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, y.shape[0], batch):
        ...
        y_batch = y[i:i+batch]
        I = ndl.Tensor(np.eye(W2.shape[1])[y_batch])
        logits = ndl.ops.relu(X_batch @ W1) @ W2
        loss = softmax_loss(logits, I)
        loss.backward()
        W1 = W1.data - lr * W1.grad.data # lr instead of lr/batch here
