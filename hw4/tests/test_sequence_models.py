...
def test_embedding():
    ...
    ndl_embedding = ndl.nn.Embedding(10, 3, device=ndl.cpu())
    torch_embedding = torch.nn.Embedding(10, 3)
    torch_embedding.weight = torch.nn.Parameter(
        torch.Tensor(ndl_embedding.weight.numpy())
    )
    input = np.array([[1, 2, 4, 5], [4, 3, 2, 9]])
    ndl_input = ndl.Tensor(input, device=ndl.cpu())
    torch_input = torch.LongTensor(input)
    ndl_output = ndl_embedding(ndl_input)
    torch_output = torch_embedding(torch_input)
    np.testing.assert_allclose(
        ndl_output.numpy(), torch_output.detach().numpy(), atol=1e-5, rtol=1e-5
    )
